...
class ExampleApp(QtWidgets.QMainWindow,clientui.Ui_MainWindow):
    ...
    def changeChanel(self):
        ...
        for message in messages:
            ...
            self.textBrowser.append('<div class="container"><h3>'+f'@{message["chanel"]}: ' + message['username'] + '</h3><p>' + message[
    ...
    def update_messages(self):
        ...
        for message in messages:
            dt = datetime.datetime.fromtimestamp(message['timestamp'])
            dt = dt.strftime('%H:%M:%S %d/%m/%Y')
            self.textBrowser.append('<div class="container"<h3>' +f'@{message["chanel"]}: '+ message['username']+'</h3><p>'+message['text']+'</p><span class="date-right">'+dt+'</span></div>')
            self.textBrowser.append('\n')
            self.last_timestamp = message['timestamp']
